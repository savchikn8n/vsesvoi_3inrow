...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const spendAmount = Math.floor(Number(amount || 0));
    ...
    const nextClaps = currentClaps - spendAmount;
    ...
    return new Response(JSON.stringify({ profile: data, spent: spendAmount }), {
      status: 200,
      headers: corsHeaders,
    });
  } catch (error) {
